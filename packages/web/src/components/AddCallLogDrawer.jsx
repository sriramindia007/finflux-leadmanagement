...
export default function AddCallLogDrawer({ leadId, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    customerPickedUp: null, leadTemp: null,
    notes: '', showFollowup: false,
  ...
  const toggleFollowup = () => {
    if (form.showFollowup) {
      // Clear fields when hiding — prevents hidden follow-up from blocking approval
      setForm(f => ({ ...f, showFollowup: false, followUpDate: '', followUpTime: '' }));
      setFollowUpError('');
    } else {
      set('showFollowup', true);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError('Failed to save call log. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const radioStyle = (active) => ({
    ...
    border: `1.5px solid ${active ? '#1874D0' : '#CFD6DD'}`,
    background: active ? '#EBF5FF' : '#fff',
    fontSize: 13, color: active ? '#1874D0' : '#374151',
    fontWeight: active ? 600 : 400,
  });
  const inputStyle = {
    padding: '8px 12px', border: '1px solid #CFD6DD',
    borderRadius: 4, fontSize: 14, color: '#003366', outline: 'none', background: '#fff',
  };
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 420, background: '#fff', zIndex: 301, display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              ...
              <input type="time" style={{ ...inputStyle, width: '100%' }} value={form.time} onChange={e => set('time', e.target.value)} />
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button style={radioStyle(form.leadTemp === 'Warm')} onClick={() => set('leadTemp', 'Warm')}>● Warm</button>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: '3px solid #1874D0', paddingLeft: 8 }}>
              ...
              <button onClick={toggleFollowup} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, color: '#1874D0' }}>
                {form.showFollowup ? '−' : '+'}
              </button>
            </div>
            {form.showFollowup && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 10 }}>
                <div>
                  ...
                  <input
                    type="date" min={today}
                    style={{ ...inputStyle, width: '100%', borderColor: followUpError ? '#EF4444' : '#CFD6DD' }}
        ...
        {/* Error message */}
        {saveError && (
          <div style={{ margin: '0 20px 8px', padding: '8px 12px', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 6, fontSize: 13, color: '#B91C1C' }}>
            {saveError}
          </div>
        )}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, padding: '16px 20px', borderTop: '1px solid #E5E7EB' }}>
          ...
          <button
            onClick={handleSave}
            disabled={saving || !!followUpError}
            style={{ padding: '8px 20px', border: 'none', borderRadius: 4, background: saving || followUpError ? '#9CA3AF' : '#1874D0', color: '#fff', cursor: saving || followUpError ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 600 }}
          >
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
