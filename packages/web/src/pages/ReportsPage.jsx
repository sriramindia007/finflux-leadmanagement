...
export default function ReportsPage() {
  ...
  const { monthly = [], sources = [], officers = [], branches = [], summary = {} } = data;
  ...
  const maxBranch  = Math.max(...branches.map(b => Number(b.total_leads || 0)), 1);
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #F3F4F6' }}>
            <SectionHeader title="By Branch" sub="Lead volume and conversion per branch office" />
          </div>
          <div style={{ padding: '8px 0' }}>
            {branches.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 32, color: '#9CA3AF', fontSize: 13 }}>No data yet</div>
              ...
              <div key={i} style={{ padding: '10px 20px', borderBottom: '1px solid #F9FAFB', display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <Bar value={row.total_leads} max={maxBranch} color="#7C3AED" />
                <div style={{ fontSize: 13, fontWeight: 700, color: '#374151', width: 30, textAlign: 'right', flexShrink: 0 }}>{fmt(row.total_leads)}</div>
                <div style={{ fontSize: 12, color: '#10B981', fontWeight: 600, width: 44, textAlign: 'right', flexShrink: 0 }}>{pct(row.conversion_rate_pct)}</div>
              </div>
            ))}
          </div>
      ...
      {/* Officer performance — full width */}
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', marginBottom: 24 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #F3F4F6' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F9FAFB' }}>
                {['Officer', 'Branch', 'Leads', 'Converted', 'Rejected', 'Conv %', 'Disbursed'].map(h => (
            ...
            <tbody>
              {officers.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32, color: '#9CA3AF', fontSize: 13 }}>No data yet</td></tr>
              ) : officers.map((row, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #F9FAFB' }}>
                  <td style={{ padding: '9px 14px', fontSize: 13, fontWeight: 600, color: '#003366' }}>{row.officer}</td>
                  <td style={{ padding: '9px 14px', fontSize: 12, color: '#6B7280' }}>{row.branch || '—'}</td>
